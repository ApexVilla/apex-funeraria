-- Corrige 409 ao baixar parcela: fin_baixar_conta_receber tentava INSERT de sessão aberta
-- sem fechar sessão aberta de outro dia (viola ux_fin_caixa_sessao_aberta_por_conta).
-- Espelha a lógica de lib/finCaixaAutoAbertura.ts (garantirSessaoCaixaParaDataBaixa).
        ...
        FOR v_old IN
            SELECT id
             ...
             WHERE conta_bancaria_id = p_conta_bancaria_id
               AND status = 'aberto'
               AND (data_abertura AT TIME ZONE 'America/Sao_Paulo')::date <> p_data
        LOOP
...
GRANT EXECUTE ON FUNCTION public.fin_caixa_garantir_sessao_para_data(
